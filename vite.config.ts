...
const entry = {
  index: resolve(import.meta.dirname, "src/inflow/index.ts"),
  portal: resolve(import.meta.dirname, "src/inflow/entries/portal.ts"),
  core: resolve(import.meta.dirname, "src/inflow/entries/core.ts"),
};
  ...
  return {
    ...
    generateBundle() {
      const sections = dependencies.map((name) => {
        ...
        const text = file ? readFileSync(resolve(dir, file), "utf8").trim() : "";
...
function withExtensions(content: string): string {
  ...
  return content.replace(specifier, (match, prefix, quote, path) =>
    /\.[cm]?[jt]sx?$|\.json$/.test(path) ? match : `${prefix}${quote}${path}.js${quote}`
  );
}
  ...
  return {
    ...
    build: {
      ...
      rollupOptions: {
        external: isCDN
          ? []
          : (id) => dependencies.some((dep) => id === dep || id.startsWith(`${dep}/`)),
        output: {
          // A legal comment, so minification keeps it.
          banner: isCDN ? "/*! See licenses of bundled dependencies in LICENSE.md */" : undefined,
        },
      },
    },
  };
