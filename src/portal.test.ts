...
describe("InflowPortal default validation messages", () => {
  ...
  // A key's first segment names the group it is written for, which is not
  // always one validator group: the address validators write into `customer.`
  // alongside the profile ones, so a key there may be served by either.
  ...
  function input(field: string, code: string) {
    ...
    // `country` is a fixed length rather than a maximum, and a long string that
    // is not an email reports `invalid_string` before it reports `too_big`.
    if (field === "country") return "xxx";
    if (field === "email") return LONG_EMAIL;
    return "x".repeat(201);
  }
  ...
  it.each(keys)("resolves %s to a message rather than the raw code", (key) => {
    ...
    const expected =
      InflowPortal.defaultTranslations[
        key as keyof typeof InflowPortal.defaultTranslations
      ];

    const candidates = groups[group]
      .map((name) => v8n[name]?.[field])
      .filter((validate) => typeof validate === "function");

    expect(candidates.length).toBeGreaterThan(0);
    expect(
      candidates.map((validate) => validate(input(field, code))),
    ).toContain(expected);
  });
  ...
  if (options.captcha) {
    ...
    element.execute = () => {
      element.dispatchEvent(
        Object.assign(new Event("verified"), { token: "captcha-token" }),
      );
    };
  }
...
describe("InflowPortal return-after-login", () => {
  ...
  it("sends the customer to the page they asked for", async () => {
    const nav = captureNavigation(
      "http://localhost:3000/sign_in.html?redirect=%2Faccount",
    );
...
// `next_transaction_date` used to parse with `z.string().datetime()`, whose
// default accepts only UTC datetimes ending in `Z`. That is neither the
// `YYYY-MM-DD` an `<input type="date">` produces nor the offset datetime the
// Customer API returns, so a form carrying this validator could never submit —
// it was invalid from first paint, holding the value the API had just sent.
describe("subscription.next_transaction_date", () => {
  afterEach(() => {
    localStorage.clear();
  });
  ...
  function validate(value: string) {
    const api = portal(BASE_A).globalContext.portal as {
      v8n: {
        subscription: { next_transaction_date: (value: string) => string };
      };
    };

    return api.v8n.subscription.next_transaction_date(value);
  }

  it.each([
    ["a date, which is what a date input produces", "2026-09-24"],
    [
      "an offset datetime, which is what the API returns",
      "2026-09-24T00:00:00-07:00",
    ],
    ["a UTC datetime", "2026-09-24T00:00:00Z"],
    ["a UTC datetime with fractional seconds", "2026-09-24T07:00:00.000Z"],
  ...
  it.each([
    ["an empty value", ""],
    ["a date in the wrong order", "24-09-2026"],
    ["a bare year", "2026"],
    ["free text", "next tuesday"],
  ])("rejects %s", (_label, value) => {
    expect(validate(value)).toBe("Next payment date is invalid.");
  });
});
