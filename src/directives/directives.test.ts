...
describe("render pipeline", () => {
  ...
  it("sets attributes via the attr- prefix and its aliases", () => {
    const inflow = mount(
      `<a data-href="url"></a><img data-attr-alt="label" />`,
    );
  ...
  it("applies every data-attr-* on one element", () => {
    const inflow = mount(
      `<a data-attr-href="url" data-attr-title="label"></a>`,
    );
    inflow.globalContext.url = "/profile.html";
    inflow.globalContext.label = "Your profile";
    inflow.render();
  ...
  it("applies every data-prop-* on one element", () => {
    const inflow = mount(
      `<input data-prop-value="email" data-prop-title="hint" />`,
    );
  ...
  it("applies every data-battr-* on one element", () => {
    const inflow = mount(
      `<input data-disabled="isBusy" data-required="isRequired" />`,
    );
  ...
  it("binds every data-on-* on one element", () => {
    const inflow = mount(
      `<input data-on-input="onInput" data-on-blur="onBlur" />`,
    );
    const onInput = vi.fn();
    const onBlur = vi.fn();
  ...
  // The value is evaluated once, at render, and the result is the listener. A
  // bare call therefore fires while the element renders rather than on the
  // event — which is why the docs tell you to wrap arguments in an arrow.
  it("evaluates a data-on-* call expression at render, not on the event", () => {
    const inflow = mount(`<button data-on-click="removeItem(7)"></button>`);
    const removeItem = vi.fn();

    inflow.globalContext.removeItem = removeItem;
    inflow.render();

    expect(removeItem).toHaveBeenCalledWith(7);
  });

  it("defers a data-on-* arrow expression to the event", () => {
    const inflow = mount(
      `<button data-on-click="() => removeItem(7)"></button>`,
    );
    const removeItem = vi.fn();
  ...
  it("inverts that condition for data-if-not", () => {
    const inflow = mount(
      `<span data-if-not="isLoggedIn">Please sign in</span>`,
    );
    inflow.globalContext.isLoggedIn = false;
    inflow.render();
