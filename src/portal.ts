...
export class InflowPortal extends InflowCore {
  static defaultTranslations = {
    ...
    "sign_up.first_name.too_big":
      "First name must not be longer than 50 characters.",
    "sign_up.first_name.too_small": "First name is required.",
    "sign_up.last_name.too_big":
    ...
    "sign_up.password.too_big":
      "Password must not be longer than 50 characters.",
    "sign_up.password.too_small": "Password is required.",
    "sign_up.password_old.too_big":
      "Password must not be longer than 50 characters.",
    "sign_up.password_old.too_small": "Current password is required.",

    // The `customer` group covers the profile and change-password pages. The
    // address validators write into this namespace too, which is why the
    // address fields appear here rather than under a namespace of their own.
    "customer.first_name.too_big":
      "First name must not be longer than 50 characters.",
    "customer.first_name.too_small": "First name is required.",
    "customer.last_name.too_big":
      "Last name must not be longer than 50 characters.",
    "customer.last_name.too_small": "Last name is required.",
    "customer.email.too_big": "Email must not be longer than 100 characters.",
    "customer.email.invalid_string": "Email is invalid.",
    "customer.email.too_small": "Email is required.",
    "customer.tax_id.too_big": "Tax ID must not be longer than 50 characters.",
    "customer.password.too_big":
      "Password must not be longer than 50 characters.",
    "customer.password.too_small": "Password is required.",
    "customer.password_old.too_big":
      "Password must not be longer than 50 characters.",
    "customer.password_old.too_small": "Current password is required.",
    "customer.address_name.too_big":
      "Address name must not be longer than 100 characters.",
    "customer.company.too_big":
      "Company must not be longer than 50 characters.",
    "customer.address1.too_big":
      "Address must not be longer than 100 characters.",
    "customer.address1.too_small": "Address is required.",
    "customer.address2.too_big":
      "Address must not be longer than 100 characters.",
    "customer.city.too_big": "City must not be longer than 50 characters.",
    "customer.region.too_big": "Region must not be longer than 50 characters.",
    "customer.postal_code.too_big":
      "Postal code must not be longer than 50 characters.",
    "customer.country.too_big": "Country must be a two-letter code.",
    "customer.country.too_small": "Country must be a two-letter code.",
    "customer.phone.too_big": "Phone must not be longer than 50 characters.",

    "default_payment_method.cc_token.too_small": "Card token is required.",

    "subscription.frequency.invalid_string":
      "Frequency must be a number followed by d, w, m or y — for example 30d.",
    "subscription.next_transaction_date.invalid_string":
      "Next payment date is invalid.",
  };
    ...
    super({
      ...
      onTokenExpiry: () => {
        ...
        const isOnSignInPage =
          signInPage.origin === location.origin &&
          signInPage.pathname === location.pathname;
      ...
      v8n: {
        signIn: {
          email: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
          ...
          password: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`sign_in.password.${error}`] || error
              : "";
          },
        },
        createAccount: {
          first_name: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`sign_up.first_name.${error}`] || error
              : "";
          },
          last_name: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`sign_up.last_name.${error}`] || error
              : "";
          },
          email: (value: string) => {
            const result = z.string().min(1).email().max(100).safeParse(value);
          ...
          password: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`sign_up.password.${error}`] || error
              : "";
          },
          password_old: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`sign_up.password_old.${error}`] || error
              : "";
          },
        },
        resetPassword: {
          email: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`reset_password.email.${error}`] || error
              : "";
          },
        },
        customer: {
          first_name: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.first_name.${error}`] || error
              : "";
          },
          last_name: (value: string) => {
            const result = z.string().min(1).max(50).safeParse(value);
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.last_name.${error}`] || error
              : "";
          },
          tax_id: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.tax_id.${error}`] || error
              : "";
          },
          email: (value: string) => {
            const result = z.string().min(1).email().max(100).safeParse(value);
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.email.${error}`] || error
              : "";
          },
          password: (value: string) => {
            const result = z.string().min(1).max(50).safeParse(value);
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.password.${error}`] || error
              : "";
          },
          password_old: (value: string) => {
            const result = z.string().min(1).max(50).safeParse(value);
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.password_old.${error}`] || error
              : "";
          },
        },
        defaultPaymentMethod: {
          cc_token: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`default_payment_method.cc_token.${error}`] ||
                  error
              : "";
          },
        },
        subscription: {
          next_transaction_date: (value: string) => {
            // A plain date is what an `<input type="date">` produces and what
            // the API accepts; an offset datetime is what the API hands back,
            // so an untouched field carries one. `z.string().datetime()` takes
            // neither — its default admits only UTC datetimes ending in `Z` —
            // which left any form using this validator unsubmittable.
            const result = z
              .string()
              .regex(
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`subscription.next_transaction_date.${error}`] ||
                  error
              : "";
          },
          frequency: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`subscription.frequency.${error}`] || error
              : "";
          },
        },
        customerAddress: {
          address_name: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.address_name.${error}`] || error
              : "";
          },
          first_name: (value: string) => {
            const result = z.string().max(50).safeParse(value);
          ...
          company: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.company.${error}`] || error
              : "";
          },
          address1: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.address1.${error}`] || error
              : "";
          },
          address2: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.address2.${error}`] || error
              : "";
          },
          city: (value: string) => {
            const result = z.string().max(50).safeParse(value);
          ...
          region: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.region.${error}`] || error
              : "";
          },
          postal_code: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.postal_code.${error}`] || error
              : "";
          },
          country: (value: string) => {
            ...
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.country.${error}`] || error
              : "";
          },
          phone: (value: string) => {
            const result = z.string().max(50).safeParse(value);
            const error =
              result.error?.issues.map((issue) => issue.code)[0] ?? "";
            return error
              ? translations[`customer.phone.${error}`] || error
              : "";
          },
        },
      },
