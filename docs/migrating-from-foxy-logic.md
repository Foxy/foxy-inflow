...
<span data-text="portal.data.customer.first_name + ' ' + portal.data.customer.last_name"></span>
```
...
**Returning to a specific page after sign-in.** Foxy Logic could send a customer back where they came from. Inflow always lands on the single `homePageUrl` you configured — after signing in and after creating an account alike. There is no per-destination option and no `?redirect=` parameter.

If you have seen `portalPageUrl` mentioned as the way to do this, disregard it: no such option exists. Use `homePageUrl` for the one destination, and if you need return-after-login, implement it outside Inflow for now.
