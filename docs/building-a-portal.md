...
| Step                                                 | Demo page                                |
| ---------------------------------------------------- | ---------------------------------------- |
| [1. A sign-in page](#1-a-sign-in-page)               | `demo/sign_in.html`                      |
| [2. Guarding a page](#2-guarding-a-page)             | `demo/index.html`                        |
| [3. Listing subscriptions](#3-listing-subscriptions) | `demo/index.html`                        |
| [4. A profile page](#4-a-profile-page)               | `demo/profile.html`                      |
| [5. Saving changes](#5-saving-changes)               | `demo/profile.html`, `demo/address.html` |
...
<div data-cloak>
  <div
    data-if="portal.data.settings.isLoading || portal.data.customer.isLoading"
  >
    Loading…
  </div>
...
<div data-source="portal.data.subscriptions">
    ...
    <tbody>
      <tr data-for="s, i in subscriptions.items">
        ...
        <td>
          ...
          <span
            data-if="s.end_date !== null"
            data-text="format.date(s.end_date)"
          ></span>
        </td>
  ...
  <p data-if="subscriptions.isReady && subscriptions.needsPagination">
    <button
      data-disabled="subscriptions.isFirstPage"
      data-on-click="subscriptions.loadFirstPage"
    >
      First
    </button>
    <button
      data-disabled="subscriptions.isFirstPage"
      data-on-click="subscriptions.loadPreviousPage"
    >
      Previous
    </button>
    ...
    <button
      data-disabled="subscriptions.isLastPage"
      data-on-click="subscriptions.loadNextPage"
    >
      Next
    </button>
    <button
      data-disabled="subscriptions.isLastPage"
      data-on-click="subscriptions.loadLastPage"
    >
      Last
    </button>
  </p>
...
`demo/address.html` applies the same pattern to a longer form, with one form-level `data-v8n="portal.v8n.customerAddress"` covering all ten fields.
